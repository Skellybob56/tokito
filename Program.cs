...
internal static class Program
{
    ...
    // todo: improve naming (Encode and PairEncode are confusing names to mean completely different things)
    static byte[] Encode(string text, string[] words, char[] punctuation)
    {
    ...
    static string Decode(byte[] tokens, string[] words, char[] punctuation)
    {
        string output = "";
    ...
    static void Main()
    {
        ...
        byte[] tokens = Encode(text, words, punctuation);
        byte[] compressed = Compress(tokens, 256 - (words.Length + punctuation.Length));

        Console.WriteLine(Decode(tokens, words, punctuation));
